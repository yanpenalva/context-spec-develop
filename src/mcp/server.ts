...
export function createCsdServer(pinnedRoot?: string): McpServer {
  const server = new McpServer(
    ...
    {
      instructions:
        'CSD MCP adapter: a transport and access boundary over the selected CSD context layout. '
        + 'Use csd_inspect first. csd_context returns the canonical bootstrap; classification, routing and the human conversation stay with you per the selected context contracts. '
        + 'Durable state lives under the selected normalized work root; persist through csd_write_artifact and verify with csd_validate. '
