...
test('bootstrap through MCP requires preview then apply with the exact token', async () => {
  ...
  const client = await connect(temp)
  try {
    const gated = await client.callTool({ name: 'csd_bootstrap_apply', arguments: { token: 'a'.repeat(64) } })
    ...
    const applied = payload(await client.callTool({ name: 'csd_bootstrap_apply', arguments: { token: preview.token } }))
